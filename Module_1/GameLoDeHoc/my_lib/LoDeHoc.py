...
def in_danh_sach_giai(danh_sach_giai):
    ...
    for giai, so in danh_sach_giai.item():
        print(f"{giai}: {so}")
    print("-" * 20)

def nhap_so_lo():
    while True:
        so_lo_input = input("Nhập số mà bạn muốn chơi, cách nhau bằng dấu phẩy: ")
...
def nhap_tien_cuoc(username, so_lo):
    ...
    while True:
        tien_cuoc = int(input("Nhập số tiền bạn muốn cược: "))
